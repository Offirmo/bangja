...
const table = new Table({
	head: ['type', 'id', 'result'],
	colWidths: [6, 55, 55] // for a 120 char term
});
...
function execute(auto_task, readonly_auto_tasks, callback) {
	...
	async.auto(readonly_auto_tasks, (err, results) => {
		...
		// TODO use cli-table
		Object.keys(results).forEach(result_key => {
			const [type, id] = result_key.split(':');
			const entry = results[result_key];
			const summary = entry.err ?
				entry.err.message :
				entry.summary || entry.data || entry;
			table.push([type, id, summary]);
		});
