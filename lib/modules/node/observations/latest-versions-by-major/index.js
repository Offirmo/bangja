	...
	return new Promise((resolve, reject) => {
		...
		versions.forEach(version => {
			...
			if (!data[major_selector]) {
				//console.log('initial set', major_selector, version);
				return data[major_selector] = version;
			}

			//console.log(version, '>', data[major_selector], '?', semver.gt(version, data[major_selector]));

			if (semver.gt(version, data[major_selector])) {
				//console.log('replace by', major_selector, version);
